...
describe("posts API", () => {
  ...
  it("should create a post", async () => {
    ...
    const postData = { content: "holis" };
    let response = await request(app)
      .post("/posts")
      .set("Authorization", `Bearer ${token}`) // Adjunto el token a la solicitud
      .send(postData);
    expect(response.statusCode).toBe(201);
    expect(response.body.data).toMatchObject(postData);
    const newId = response.body.data.username;
    response = await request(app).get(`/posts/${newId}`);
    expect(response.statusCode).toBe(200);
  });
